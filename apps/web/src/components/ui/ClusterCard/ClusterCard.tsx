...
export const ClusterCard = ({
    ...
    return (
        <Component
            className={classesJoined([className, 'ui-cluster-card', onClick ? 'ui-cluster-card--interactive' : 'ui-cluster-card--interactive'])}
            onClick={clickHandler}
            type={onClick ? 'button' : undefined}
        >
            {imageUrl && (
                <div className='ui-cluster-card__image-wrapper'>
                    <img src={imageUrl} alt={title} className='ui-cluster-card__image' />
                </div>
            )}
